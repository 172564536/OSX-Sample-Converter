...
NSString * const DEFS_KEY_PRESERVE_FOLDER_HIERARCHY  = @"mpc.defs.preserveFolderHierarchy";
...
+(void)setUpDefaultValuesIfPlistMissing
{
    ...
    if ([[defs allKeys]count] == 0) {
        ...
        [defs setValue:@YES forKey:DEFS_KEY_PRESERVE_FOLDER_HIERARCHY];
        [defs writeToFile:[self getPath] atomically:YES];
    } else {
        ...
        if (![defs valueForKey:DEFS_KEY_PRESERVE_FOLDER_HIERARCHY]) {
            [defs setValue:@YES forKey:DEFS_KEY_PRESERVE_FOLDER_HIERARCHY];
        }
        [defs writeToFile:[self getPath] atomically:YES];
    }
